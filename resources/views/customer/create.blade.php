...
@section('content')
    <h1>Create customer</h1><br>
    <a href="{{ route('customers.index') }}" class="btn btn-primary">Go Back</a><br><br>
    <form action="{{ route('customers.store') }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="form-group">
            <label for="username">Name</label>
            <input type="text" name="name" class="form-control" id="username" aria-describedby="nameHelp" value="{{ old('name') }}" placeholder="Enter a name">
            @error('name')
        ...
        <div class="form-group">
            ...
            <textarea rows="3" name="location" class="form-control" id="location" aria-describedby="loHelp" placeholder="Enter your location">{{ old('location') }}</textarea>
            @error('location')
                <small id="loHelp" class="form-text text-danger">{{ $message }}</small>
            @enderror
        </div><br>
